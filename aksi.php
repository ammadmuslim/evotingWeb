...
require_once'functions.php';
    ...
    if ($mod=='login_pemilih'){
        ...
        $row = $db->get_row("SELECT * FROM tb_pemilih WHERE nis='$user' AND pass='$pass'");
        if($row){
            $_SESSION[login] = TRUE;
            ...
            print_msg("Salah kombinasi NIS dan password.");
        }          
    }elseif ($mod=='login'){
        $user = esc_field($_POST[user]);
        $pass = esc_field($_POST[pass]);
    ...
    if($mod=='pemilih_tambah'){
        ...
        if(!$nis || !$nama_pemilih || !$pass)
            print_msg("Field bertanda * tidak boleh kosong!");
        elseif(strlen($pass)<4 || strlen($pass)>16)
            print_msg("Password 4-16 karakter!");
        elseif($db->get_results("SELECT * FROM tb_pemilih WHERE nis='$nis'"))
            print_msg("No NIS sudah terdaftar!");
        else{
            $db->query("INSERT INTO tb_pemilih (nis, nama_pemilih, alamat, pass) VALUES ('$nis', '$nama_pemilih', '$alamat', '$pass')");
        ...
        elseif(strlen($pass)<4 || strlen($pass)>16)
            print_msg("Password 4-16 karakter!");
        elseif($db->get_results("SELECT * FROM tb_pemilih WHERE nis='$nis' AND id_pemilih<>'$_GET[ID]'"))
            print_msg("No NIS sudah terdaftar!");
        else{
            $db->query("UPDATE tb_pemilih SET nis='$nis', nama_pemilih='$nama_pemilih', alamat='$alamat', pass='$pass' WHERE id_pemilih='$_GET[ID]'");
            redirect_js("index.php?m=pemilih");
        }    
